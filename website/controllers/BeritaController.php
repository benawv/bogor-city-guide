...
class BeritaController extends Website_Controller_Action {
	...
	public function homeEnAction () {
		...
		//$list->setOrder("desc");
		$sql = "SELECT doc.id, doc_e.data from documents as doc inner join documents_elements as doc_e on doc.id=doc_e.documentId
				where doc.path LIKE '%/news-archive/' and doc.published=1 and doc_e.type='date'
				ORDER BY doc_e.data DESC limit 3"; //or whatever you need to do.
		/* echo "<pre>";
		print_r($db->fetchAll($sql));
		die(); */
	...
	public function loadMoreEnAction()
	{
		$offset = $_POST["offset"];
		$create = $_POST["created_at"];
		...
		if($create!='')
		{
			$sql .= "where doc.path LIKE '%/news-archive/' and DATE_FORMAT(FROM_UNIXTIME(doc_e.data), '%Y') = ".$create." and doc.published=1 and doc_e.type='date' ORDER BY doc_e.data DESC limit 3 offset ".$offset;
		}
		else 
		{
			$sql .= "where doc.path LIKE '%/news-archive/' and doc.published=1 and doc_e.type='date' ORDER BY doc_e.data DESC limit 3 offset ".$offset;
		}
	...
	public function filterTahunEnAction()
	{
		$create = $_POST["created_at"];
		$db = Pimcore_Resource_Mysql::get();
		$sql = "SELECT doc.id, doc_e.data from documents as doc inner join documents_elements as doc_e on doc.id=doc_e.documentId ";
		if($create!='')
		{
			$sql .= "where doc.path LIKE '%/news-archive/' and DATE_FORMAT(FROM_UNIXTIME(doc_e.data), '%Y') = ".$create." and doc.published=1 and doc_e.type='date' ORDER BY doc_e.data DESC limit 3";
		}
		else
		{
			$sql .= "where doc.path LIKE '%/news-archive/' and doc.published=1 and doc_e.type='date' ORDER BY doc_e.data DESC limit 3";
		}
