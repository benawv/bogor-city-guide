...
class KuisController extends Website_Controller_Action {
	...
	public function notifGagalAction(){
		$db = Pimcore_Resource_Mysql::get();
		
		$peserta = new Object_DataPesertaAJFC_List();
		$peserta->setCondition("(emailgagal <> 1 OR emailgagal is null) AND lulus <> 1 AND totalNilai IS NOT NULL AND totalNilai > 0");
		$peserta->setLimit(20);
		...
		foreach($peserta as $row)
		{
			...
			$id = $row->getId();
			$data = Object_DataPesertaAJFC::getById($id);
			$data->setEmailgagal(1);
		
			$document = '/email/email-gagal';
			$mail = new Pimcore_Mail();
			$mail->setSubject("Mohon Maaf Anda Tidak Lolos AJFC 2015");
			$mail->setFrom("no-reply@ajfc.allianz.co.id","Allianz Junior Football Camp 2015");
			$mail->setDocument($document);
			$mail->addTo($row->getEmail());
	...
	public function notifinfotambahanAction(){
		...
		foreach($peserta as $row)
		{
			...
			$document = '/email/email-tambahan';
			
			$mail = new Pimcore_Mail();
			$mail->setSubject("Pengumuman Tambahan AJFC 2015");
			$mail->setFrom("no-reply@ajfc.allianz.co.id","Allianz Junior Football Camp 2015");
			$mail->setDocument($document);
	...
	public function notifSalahKirimAction(){
		...
		foreach($peserta as $row)
		{
			...
			$mail = new Pimcore_Mail();
			$mail->setSubject("Kesalahan Pengiriman Email AJFC 2015");
			$mail->setFrom("no-reply@ajfc.allianz.co.id","Allianz Junior Football Camp 2015");
			$mail->setDocument($document);
