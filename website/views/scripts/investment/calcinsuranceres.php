...
<?php $session = new Zend_Session_Namespace('HitIns'); ?>
	...
	$('#resp1_biaya').html(accounting.formatMoney(<?php echo $session->biaya;?>,'Rp ',2,'.',','));
	var resp1_asumsi_imbal = getCookie('resp1_asumsi_imbal');
	$('#resp1_asumsi_imbal').html(<?php echo $session->asumsiimbal; ?> + '%');
	...
	$(document).ready(function(){
		...
		$('.sendEmail').click(function(){
			if($('.email-user').val() != "")
			{
                var biaya = '<?php echo $session->biaya; ?>';
                var result = '<?php echo $session->result; ?>';
                var asumsiinflasi = '<?php echo $session->asumsiinflasi; ?>';
                var asumsiimbalan = '<?php echo $session->asumsiimbal; ?>';
                var waktu = '<?php echo $session->waktu; ?>';
				$.ajax({
					type: 'POST',
					url: '/sendkalkulator',
					data: {
						kalkulator : 'Asuransi',
						biaya: accounting.formatMoney(biaya,'Rp ',2,'.',','),
						result_1: accounting.formatMoney(result,'Rp ',2,'.',','),
						asumsi_inflasi: asumsiinflasi,
						asumsi_imbalan : asumsiimbalan,
						jangka_waktu: waktu,
						risiko: asumsiimbalan,
    ...
    <?php Zend_Session::namespaceUnset('HitIns'); ?>